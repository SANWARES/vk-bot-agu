...
def find_prepod_by_fio(message):
    output = ""
    teacher_name = ""
    teacher_dict = teacher()
    schedule_teacher = re.search(r'([A-Za-zА-ЯЁа-яё .]+)', message)

    if schedule_teacher is None:
        output = "Не найдено ни одного преподавателя. Если преподаватель с данными инициалами " \
    ...
    elif schedule_teacher is not None:
        count = 0
        teachers = ""
        for i in teacher_dict:
            ...
            if xD is not None:
                count += 1
                teachers += "-> " + teacher_dict[xD[0]][2] + " (%s)" % \
                            teacher_dict[xD[0]][3] + "\n"
                teacher_name = xD[0]
        if count == 0:
            output = "Не найдено ни одного преподавателя. Если преподаватель с данными " \
                     "инициалами действительно существует, то попробуйте сократить запрос. " \
                     "Обратите внимание, что фамилию тоже можно писать не всю"
            # handling_keyboard = self.read_keyboard("return.json")

        elif count > 10:
            output = "Слишком много преподавателей по вашему запросу. " \
                     "Напишите более полные данные."
            # handling_keyboard = self.read_keyboard("return.json")

        elif count != 1:
            try:
                output = "Слишком много преподавателей по вашему запросу. Сократите " \
                         "поиск до одного преподавателя\n\n" + teachers
                # handling_keyboard = self.read_keyboard("return.json")
            except:
                output = "Попробуйте изменить свой запрос указав более " \
                         "полные инициалы преподавателя"

        else:
            output = "500"

    return output, teacher_name


def find_ref_group_by_name(division, group_name):
    "Поиск ида группы по имени"

    reg_group = ""
    for i in Group_1_back()[division].items():
        if i[1] == group_name:
            reg_group = i[0]
